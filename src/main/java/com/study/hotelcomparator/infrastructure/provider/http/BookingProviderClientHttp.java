...
@Service
@Profile("http")
public class BookingProviderClientHttp implements ProviderClient {
    private final ProviderHttpClient httpClient;
    private final ProviderRowsMapper rowsMapper;
    private final String baseUrl;
    private final String path;

    public BookingProviderClientHttp(ProviderHttpClient httpClient,
                                     ProviderRowsMapper rowsMapper,
                                     @Value("${providers.base-url}") String baseUrl,
                                     @Value("${providers.booking-path}") String path) {
    ...
    public List<HotelQuote> fetchOffers(String city,
                                        LocalDate checkIn,
                                        LocalDate checkOut) {
        String apiUrl = httpClient.buildUrl(baseUrl, path);
        List<ProviderHotelPriceRow> rows = httpClient.getRows(apiUrl);
        return rowsMapper.toHotelQuotes(city, rows, Provider.BOOKING);
    }
}
